...
| 0.13.0 | 2026-08-23 | Фильтр сессий в сайдбаре: поиск по имени/превью + «только избранные» (`.session-filter`) |
| 0.13.0 | 2026-08-23 | Свёрнутый трейс шагов (`.agent-trace`) после завершения агент-запуска — френдли-шаги не теряются |
| 0.13.0 | 2026-08-23 | Трейс переживает переключение сессий: `history` шлёт items (user/assistant+steps), `restoreHistory` восстанавливает `.agent-trace` |
| 0.13.0 | 2026-08-23 | Кнопка «⏹️ Остановить» → текстовая (убрана `.icon-btn` 28px — текст не обрезается, без дублирующего tooltip) |
