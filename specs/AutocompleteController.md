...
|--------|------|-----------|
| 0.13.0 | 2026-08-23 | Async-провайдер: LLM-запрос в `provideInlineCompletionItems`; фолбэк провайдера (`getDefault` → первый настроенный) и модели (`defaultModel` → первая у провайдера), иначе ghost text молча не появлялся |
