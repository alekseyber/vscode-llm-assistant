...
- **Формат чанка:** `data: {"choices":[{"delta":{"content":"токен"}}]}\n\n`
- **Завершение:** `data: [DONE]\n\n`
- **Разделители чанков:** двойной `\n\n`
- **Комментарии:** строки с `:` (двоеточие) — игнорируются
- **Невалидный JSON:** чанк пропускается, не роняет поток
- **Многострочные данные:** значение поля data может содержать `\n`

## Форматы данных

### SSE-чанк
```
data: {"id":"chatcmpl-xxx","object":"chat.completion.chunk","choices":[{"delta":{"content":"привет"}}]}\n\n
```
