...
- **Ход выполнения (`toolActivity`):** вызовы тулов и их результаты (`onStep`, `onConfirm`) отправляются в WebView как `toolActivity`, а не `streamChunk` — отдельный буфер `streamingActivity` показывается во время работы агента и скрывается по завершении (`done`); в финальный ответ (`streamingRawText`) попадает только `result.answer`.
