    ...
    if (providerName) {
      ...
      if (providerCfg.systemPrompt) {
        ...
        prompt = config.get<string>('chat.agentSystemPrompt') ||
          ...
          'НЕ ОТВЕЧАЙ текстом там, где нужно вызвать инструмент. НЕ задавай уточняющих вопросов без явной просьбы пользователя. Если задача понятна — выполняй. Отвечай кратко, по-русски.';
      } else {
